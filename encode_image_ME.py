...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='',
                        help='Directory to save the results. If not specified, '
                             '`./outputs/encodings` will be used by default.')
...
def main():
    ...
    for it, image_id in tqdm(enumerate(range(0, input_images.shape[0], args.batch_size))):
        ...
        for i in range(args.batch_size):
            id_num = 8*it + i
            np.save(os.path.join(save_dir, '%05d_w.npy' % (id_num)), encodings[i])
        
        #temp.append(encodings)
        #print(it)
    ...
    #temp = np.array(temp)
    #final_enc = temp.reshape(-1, temp.shape[2])
